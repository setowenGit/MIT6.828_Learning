...
//
// 温馨提示： 考虑在同一个pgdir中的同一个虚拟地址重新插入同一个pp时会发生什么.
...
//
// 检查权限为环境env在[va, va+len)是否有权限'perm | PTE_P'的访问.
// 通常 prem 至少都会包含 PTE_U, 虽然这并不是必须要含有的
// 'va'和'len'参数本身并不要求与页面对齐的;你必须测试包含该范围内的任何一页.
...
// 如果发送错误，将'user_mem_check_addr'赋值为第一个有错误的虚拟地址.
//
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm) {
    // LAB 3: Your code here.
    uintptr_t start = (uintptr_t) ROUNDDOWN(va, PGSIZE); //注意：这里是ROUNDDOWN函数, 如果是ROUNDUP 就有可能忽略一个页了!
    ...
    for (; start < end; start += PGSIZE) {
        ...
        if (start >= ULIM || pte == NULL || (*pte & perm) != perm) {
            user_mem_check_addr = (uintptr_t) start < (uintptr_t) va ? (uintptr_t) va : start; //因为start使用了ROUNDDOWN,则有可能地址比va小.
            return -E_FAULT;
        }
    }
    return 0;
...
//
// 检查环境 "env "访问内存范围[va, va+len)的权限是否符合'perm | PTE_U | PTE_P'.
// 如果它符合,那么这个函数就会简单地返回.
// 如果不符合,'env'就会被销毁, 如果env是当前环境,那么这个函数将不会返回.
void
