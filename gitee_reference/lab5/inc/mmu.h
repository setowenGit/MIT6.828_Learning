...
struct Taskstate {
	...
	uintptr_t ts_esp0;	// Stack pointers and segment selectors 堆栈指针和段选择器
...
// Set up a normal interrupt/trap gate descriptor. 设置一个正常的中断/陷阱门描述符
// - istrap: 1 for a trap (= exception) gate, 0 for an interrupt gate. 表示陷阱（=异常）门，0表示中断门
