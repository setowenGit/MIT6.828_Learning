...
// Top of user-accessible VM 用户可访问的虚拟机的顶部
#define UTOP		UENVS
 ...
 *
 * 与虚拟地址范围相对应的PDE
 * [UVPT, UVPT + PTSIZE)指向页目录本身。 因此，该页
 * 目录被看作是一个页表以及一个页目录。
 *
 * 将页目录作为页表处理的一个结果是，所有的PTEs
 * 可以通过虚拟地址UVPT的 "虚拟页表 "来访问。
 * uvpt在lib/entry.S中被设置为）。 页号为N的PTE被存储在
 * uvpt[N]。 (这值得画一张图！)
 *
 * 第二个结果是，当前页面目录的内容
 * 将总是在虚拟地址(UVPT + (UVPT >> PGSHIFT))上可用，这个地址是
