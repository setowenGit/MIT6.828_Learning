...
env_setup_vm(struct Env *e)
{
    ...
    memcpy((void *)e->env_pgdir, kern_pgdir, PGSIZE); //继承内核页目录
	p->pp_ref++;
...
//
// 为env环境 分配 len个字节物理地址内存, 并且将环境地址空间映射到虚拟地址va。
// 不要
static void
...
//
//所有这些都与我们的引导加载程序非常相似，只是引导加载程序还需要从磁盘读取代码。查看boot/main。c来获得想法。
//
//最后，此函数映射程序初始堆栈的一个页面。
//
...
env_create(uint8_t *binary, enum EnvType type)
{
    ...
    if (type == ENV_TYPE_FS){
        new_e->env_tf.tf_eflags |= FL_IOPL_3;
    }
}
