...
trap_init(void)
{
	...
	SETGATE(idt[IRQ_OFFSET + IRQ_IDE], 0, GD_KT, irq_14_handler, 0);
...
trap(struct Trapframe *tf)
{
	...
	// sched_yield()
	if (xchg(&thiscpu->cpu_status, CPU_STARTED) == CPU_HALTED) //利用了CAS的思想，而java的CAS用的是 lock cmpxchg 指令
		lock_kernel();
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    //我们已经处理了内核模式的异常，因此如果我们到达这里，页故障发生在用户模式中。
    //
    //如果存在环境的页故障处理程序（page fault upcall），则调用它。在用户异常堆栈上（UXSTACKTOP以下）设置一个页故障堆栈帧，然后跳转到curenv->env_pgfault_upcall。
    //
    //页故障处理程序可能会导致另一个页故障，这种情况下我们会递归地跳转到页故障处理程序，并在用户异常堆栈顶部添加另一个页故障堆栈帧。
    //
    //对于我们的代码来说，在处理页故障返回（lib/pfentry.S）时，在陷阱时间堆栈的顶部留出一个字的空间非常方便；这使我们可以更容易地恢复eip/esp。
    // 在非递归情况下，我们不必担心这个问题，因为常规用户堆栈的顶部是空闲的。
    // 在递归情况下，这意味着我们必须在当前异常堆栈顶部和新的堆栈帧之间留出一个额外的字，因为异常堆栈本身是陷阱时间堆栈。
    //
    //如果没有页故障处理程序，那么环境没有为其异常堆栈分配页面，或无法写入该页面，或异常堆栈溢出，则销毁导致故障的环境。
    // 注意，测试脚本假定您首先检查页故障处理程序，并且如果没有，则打印“user fault va”消息。可以将其余三个检查合并为一个测试。
    //
    //提示：
    //user_mem_assert()和env_run()在此处非常有用。
