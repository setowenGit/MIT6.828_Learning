...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// entry.S页面目录映射前4MB的物理内存


// entry.S页面目录从虚拟地址KERNBASE开始映射前4MB的物理内存
//（也就是说，它将虚拟地址[KERNBASE, KERNBASE+4MB]映射到物理地址[0, 4MB]）。
// 我们选择4MB是因为这就是我们可以用一个页面映射的数量
// 表来映射的数量，而且它足以让我们通过早期启动。 
// 我们还将虚拟地址[0, 4MB]映射到物理地址[0, 4MB]；
// 这个区域对于entry.S中的一些指令来说是至关重要的，然后我们就不再使用它了。
// 页目录（和页表），必须在页边界上开始，因此有"__aligned__"属性。 
// 另外，由于与链接和静态初始化器有关的限制，我们在这里使用 "x + PTE_P"，
// 而不是更标准的 "x | PTE_P"。 在其他地方，你应该使用"|"来组合标志。
