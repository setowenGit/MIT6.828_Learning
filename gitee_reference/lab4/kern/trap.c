...
// Initialize and load the per-CPU TSS and IDT 初始化并加载每个cpu的TSS and IDT
void
trap_init_percpu(void)
{
	...
	// 这里的代码是针对于CPU0设置了TSS和TSS的描述信息，但是如果我们让其它CPU也启动的话 那么这会是错误的
    // 因为每个CPU都应该拥有自己的内核栈
    // 为了让所有CPU都能运行起来，赶快修复它
    //
	// Hints:
	...
	// ltr在TSS选择器中设置了一个 "busy"的标志，所以如果你不小心在一个以上的CPU上加载同一个TSS，
    // 你会得到一个三重故障。 如果你错误地设置了一个单独的CPU的TSS，直到你试图从该CPU的用户空间返回之前 你可能都不会出现故障，
    //
...
page_fault_handler(struct Trapframe *tf)
{
	...
	fault_va = rcr2();// cr2寄存器存储的是 引起错误的虚拟地址
	...
	if (curenv->env_pgfault_upcall) {
		...
		env_run(curenv); //The function do not return
	}
