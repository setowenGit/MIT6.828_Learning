...
#define COM1		0x3F8// 串口基地址

#define COM_RX		0	// In:	Receive buffer (DLAB=0)
#define COM_TX		0	// Out: Transmit buffer (DLAB=0) 发送寄存器
...
#define   COM_LSR_TXRDY	0x20	//   Transmit buffer avail 用于判断串口是否准备就绪
...
lpt_putc(int c)
{
	...
	for (i = 0; !(inb(0x378+1) & 0x80) && i < 12800; i++)
		delay();
	outb(0x378+0, c); // 将字符 c 输出到并行端口的数据寄存器
	// 设置并行端口控制寄存器的值，以控制打印操作
