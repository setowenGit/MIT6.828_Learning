...
env_setup_vm(struct Env *e)
{
    ...
    memcpy((void *) e->env_pgdir, kern_pgdir, PGSIZE); //复制内核页目录, 因为在UTOP之上与kern_pgdir是一样的，所以可以直接把kern_pgdir的内容全部拷贝过来
    p->pp_ref++;
...
region_alloc(struct Env *e, void *va, size_t len)
{
    ...
    for (void *curVa = begin; curVa < end; curVa += PGSIZE) {
        ...
        if (page_insert(e->env_pgdir, p, curVa, PTE_U | PTE_W | PTE_P)) {
            panic("region alloc error! error:%e", -E_NO_MEM);
        }
    }
