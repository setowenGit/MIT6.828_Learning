...
boot_alloc(uint32_t n)
{
	...
	// 'end'是一个由链接器自动生成的指向内核bss段的末端的神奇符号，它是链接器没有分配给任何内核代码或全局变量的第一个虚拟地址
	if (!nextfree) {
		extern char end[];
...
page_alloc(int alloc_flags)
{
	...
	pp->pp_link = NULL; // 为了使page_free正常运行和准确判断
...
//对条目使用权限位perm | PTE_P。
//
//此函数仅用于在UTOP上设置“静态”映射。因此，它不应更改映射页面上的pp_ref字段。
//
...
//
// 使TLB条目失效，但只有在被编辑的页表是处理器当前使用的页表时才会失效
void
